...
RESUME_FILENAME = 'claims_resume_file.txt'
...
print('Loading claims_df pickle...')
claims_df = load_pickle('./newest_claims_df_labelled.pkl').dropna()
...
true_bert_mat = np.vstack(list(true_claims_df['bert_embedding']))
false_bert_mat = np.vstack(list(false_claims_df['bert_embedding']))
misleading_claims_bert_mat = np.vstack(list(misleading_claims_df['bert_embedding']))
false_bert_mat = np.vstack([false_bert_mat, misleading_claims_bert_mat])

true_ft_mat = np.vstack(list(true_claims_df['ft_embedding']))
false_ft_mat = np.vstack(list(false_claims_df['ft_embedding']))
misleading_claims_ft_mat = np.vstack(list(misleading_claims_df['ft_embedding']))
false_ft_mat = np.vstack([false_ft_mat, misleading_claims_ft_mat])
...
def baseline_run(dset_name, method_name, embedding_type):
    if already_done():
        return True
    print('Running baseline: ', (dset_name, method_name, embedding_type))
    args.num_cord = 0
...
def cord_run(dset_name, method_name, embedding_type, num_cord):
    if already_done():
        return True
    print('Running CORD: ', (dset_name, method_name, embedding_type, num_cord))
...
# Train baselines
for k in range(3):
    baseline_run('claims_fixed', 'logistic_regression', 'bert')
    baseline_run('claims_fixed', 'random_forest', 'bert')
    baseline_run('claims_fixed', 'logistic_regression', 'fasttext')
    baseline_run('claims_fixed', 'random_forest', 'fasttext')


TARGET = [10, 100, 1000, 5000, 10000]
for k in range(3):
    for num_cord in TARGET:
        cord_run('claims_fixed', 'logistic_regression', 'bert', num_cord)
        cord_run('claims_fixed', 'random_forest', 'bert', num_cord)
        cord_run('claims_fixed', 'logistic_regression', 'fasttext', num_cord)
        cord_run('claims_fixed', 'random_forest', 'fasttext', num_cord), num_cord
