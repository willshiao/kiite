...
DATASET_DIR = '/media/will/SSD Storage 2/research/trusted_news_all'
# DATASET_DIR = '/mnt/fusion/Research/covid-fake-news/crawler'
DATASET_PATH = path.join(DATASET_DIR, 'output')
...
with open('trustworthy.txt', 'r') as f:
...
other_len = len(meta_hindex['other'])
print(f'{other_len} other sites found: ', meta_hindex['other'])
...
for host, files in tqdm(full_index.items()):
    with open(path.join(SHORT_DIR, host.replace('.', '_') + '.json'), 'w') as f:
        with Pool(30) as pool:
            for val in tqdm(pool.imap(clean_file, files), total=len(files)):
